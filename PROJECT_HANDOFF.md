...
|---|---:|
| `submission_swin80_thbtd50_hg40_cf_ff.zip` | `0.6643951440975752` |
...
## フレーム欠損バグ発見とfix（2026-07-07、新best −0.00008）

EDA継続で発見: **eval 604行（2.1%）、train 890行がフレーム不足**（3枚未満）。系統性あり — 欠けるのは主に **t-10（最新フレーム）**、Himawariは分単位:40/:50に集中（衛星ハウスキーピング由来）。さらに現行実装は位置詰めパディングのため、**t-20欠け行では t-10 が誤ったスロットに入る時間軸ずれ**も発生（eval 177行）。

- **fix**: タイムスタンプ整列 + 欠けスロットは最近傍フレーム複製（差分チャネルの偽ジャンプが消える）。推論時のdataframe加工のみ（`observation_files` リストを書き換え）、モデル・本線コード変更なし
- **train OOF検証（655行）**: two_head mix tile RMSE −0.0066（th-fixのみが最良）。**btd は +0.0012、wh は +0.0044 で fix すると悪化** → ゴミ入力前提で学習したモデルは推論時だけ直すと分布ずれ。**th成分のみ適用**
