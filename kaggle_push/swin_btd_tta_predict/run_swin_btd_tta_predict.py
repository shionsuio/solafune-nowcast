...
def main() -> None:
    ...
    for fold in range(5):
        ...
        np.save(
            f"/kaggle/working/prediction_btd_tta_fold{fold}.npy",
    ...
    np.save("/kaggle/working/prediction_btd_tta_ensemble.npy", predictions.astype(np.float16))
    print(
        f"ensemble mean={predictions.mean():.6f} "
        f"p95={np.quantile(predictions, 0.95):.6f} max={predictions.max():.6f}"
    )

    output_zip = Path("/kaggle/working/submission_swin_btd_tta_5fold.zip")
