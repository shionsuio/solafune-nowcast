...
def copy_model_inputs() -> tuple[Path, Path]:
    ...
    th_checkpoints = [p for p in checkpoints if "two_head_full" in str(p).lower().replace("-", "_")]
    th_stats = [p for p in stats if "two_head_full" in str(p).lower().replace("-", "_")]
    print("two_head_full checkpoints:", len(th_checkpoints), "btd stats:", len(th_stats))
    assert len(th_checkpoints) == 5 and len(th_stats) == 5

    model_dir = Path("/kaggle/working/models/swin_v2_temporal_two_head_full")
    stats_dir = Path("/kaggle/working/outputs/band_stats_two_head_full")
    model_dir.mkdir(parents=True, exist_ok=True)
    stats_dir.mkdir(parents=True, exist_ok=True)
    for path in th_checkpoints:
        shutil.copy2(path, model_dir / path.name)
    for path in th_stats:
...
def main() -> None:
    ...
    evaluation_frame = align_observation_frames(evaluation_frame)
    device = torch.device("cuda")
    prediction_sum = None
    for fold in range(5):
        fold_predictions = predict_fold(config, evaluation_frame, fold, device, tta=True)
        np.save(
            f"/kaggle/working/prediction_two_head_tta_fold{fold}.npy",
    ...
    np.save("/kaggle/working/prediction_two_head_tta_ensemble.npy", predictions.astype(np.float16))
    print(
        f"ensemble mean={predictions.mean():.6f} "
        f"p95={np.quantile(predictions, 0.95):.6f} max={predictions.max():.6f}"
    )

    output_zip = Path("/kaggle/working/submission_swin_two_head_tta_5fold.zip")
